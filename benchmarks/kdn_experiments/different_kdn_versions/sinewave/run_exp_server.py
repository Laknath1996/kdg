...
from kdg.utils import generate_sinewave as generate_data
...
k = float(args['k'])
reps = int(args['reps'])
print("Running the Sinewave experiment...")
...
# network architecture [15, 15, 15, 15, 2]
def getNN():
    network_base = keras.Sequential()
    initializer = keras.initializers.GlorotNormal(seed=0)
    network_base.add(keras.layers.Dense(15, activation="relu", kernel_initializer=initializer, input_shape=(2,)))
    network_base.add(keras.layers.Dense(15, activation="relu", kernel_initializer=initializer))
    network_base.add(keras.layers.Dense(15, activation="relu", kernel_initializer=initializer))
    network_base.add(keras.layers.Dense(15, activation="relu", kernel_initializer=initializer))
